...
public class CloudStackEC2AMIClientLiveTest extends AMIClientLiveTest {
    ...
    private String templateArchitecture;
    ...
    protected Properties setupProperties() {
        ...
        String ebsSpec = checkNotNull(setIfTestSystemPropertyPresent(overrides, provider + ".ebs-template"), provider
                + ".ebs-template");
        templateURL = checkNotNull(setIfTestSystemPropertyPresent(overrides, provider + ".template-url"), provider
                + ".template-url");
        templateArchitecture = checkNotNull(setIfTestSystemPropertyPresent(overrides, provider + ".template-architecture"), provider
    ...
    public void testRegisterImage(){
        ...
        options.asArchitecture(templateArchitecture);
        registeredImageId = cloudstackClient.registerImageFromManifestInRegion(regionId,"imageRegister",
                templateURL,
                new CloudStackEC2RegisterImageOptions().asArchitecture(templateArchitecture)) ;
    }
    ...
    public void testCreateAndListEBSBackedImage() throws Exception {
        throw (new SkipException(" done differently in CloudStack"));
    }

    @Override
    @Test(dependsOnMethods = "testCreateAndListEBSBackedImage")
    public void testGetLaunchPermissionForImage() {
       throw (new SkipException(" dependent on testCreateAndListEBSBackedImage which is not supported for CloudStack"));
    }
